...
export default function Filters({ onFilter }: asideProps<any>) {
  ...
  const { filters } = useAppSelector((s) => s.draw.won);
  ...
  return (
    <Formik
      initialValues={{
        from: moment(filters?.from || minDate).format("YYYY-MM-DD"),
        to: moment(filters?.to || maxDate).format("YYYY-MM-DD"),
      ...
      {({ values, resetForm, isSubmitting, handleSubmit, handleChange, setValues }) => (
        <form className="grid gap-3" onSubmit={handleSubmit}>
          ...
          <Input
            name="tokenId"
            placeholder="Token"
            label={<p className="-mb-2">Token</p>}
          ...
          <Input
            name="search"
            placeholder="Search"
            label={<p className="-mb-2">Search</p>}
            value={values?.search}
            onChange={handleChange}
          />
